 ...
 * @Test void successUpdateConnectStatus() { // given User user = User.builder() .actualUserId(1L) .tier(Tier.gold)
 * .division(3) .summonerName("uni") .status(Status.ONLINE) .lp(3L).build(); userRepository.save(user);
 * <p>
 * // when stompService.updateConnStatus(user, Status.OFFLINE);
 * <p>
 * // then User findUser = userService.getUser(1L); assertThat(findUser.getStatus()).isEqualTo(Status.OFFLINE); } }
 * <p>
 * }
 **/